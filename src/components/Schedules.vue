  ...
  <div class="schedules-container">
    <div class="schedule-list">
      <h1 class="text-center" style="color: white">
        <span class="orange-underline"> Welcome, Admin </span>
      </h1>
      <div>
        <!-- Search box -->
        ...
        <div
          class="button_cont"
          style="margin: 3%; display: flex; justify-content: center"
        >
          <a
            style="width: 75%"
            class="create"
        ...
        <ul style="display: flex; flex-wrap: wrap; margin: 0; padding: 0">
          ...
          >
            <div
              class="flex shadow-sm border justify-content-center text-center align-items-center h-screen m-1 rounded p-3"
              ...
              <div class="rounded overflow-hidden">
                <div class="p-l-6 pt-4 p-r-6">
                  ...
                  <label class="switch">
                    <input
                      type="checkbox"
                      :disabled="schedule.displayed"
                      v-model="schedule.displayed"
                      v-on:change="publishSchedule(schedule)"
                    />
                    <span class="slider round"></span>
                  </label>
                </div>
                <!-- View, Edit, and delete schedule -->
                <div style="display: flex; flex-direction: column">
                  ...
                  <!-- <button class="downloadpdf" @click="downloadpdf(schedule)">
                    Download As PDF
                  </button>
 -->
                  <button
...
export default {
  ...
  methods: {
    /* Publish schedule and set as active */
...
/* UNFINISHED: Create PDF/DOCX from schedule */
    /* downloadpdf(item) {
      //210 297
      var doc = new jsPDF();
      //HEADER
      ...
      doc.text(
        item.phoneNumbers[0].teacherName +
          "   " +
      ...
      doc.text(
        item.phoneNumbers[1].teacherName +
          "   " +
          item.phoneNumbers[1].phoneNumber,
        68,
        34,
        { align: "right" }
      );
      doc.text(
        item.phoneNumbers[2].teacherName +
          "   " +
          item.phoneNumbers[2].phoneNumber,
        68,
        40,
        { align: "right" }
      );
      doc.text(
        item.phoneNumbers[3].teacherName +
          "   " +
          item.phoneNumbers[3].phoneNumber,
        135,
        28,
        { align: "right" }
      );
      doc.text(
        item.phoneNumbers[4].teacherName +
          "   " +
          item.phoneNumbers[4].phoneNumber,
        135,
        34,
        { align: "right" }
      );
      doc.text(
        item.phoneNumbers[5].teacherName +
          "   " +
          item.phoneNumbers[5].phoneNumber,
        135,
        40,
        { align: "right" }
      );
      doc.text(
        item.phoneNumbers[6].teacherName +
          "   " +
          item.phoneNumbers[6].phoneNumber,
        205,
        28,
        { align: "right" }
      );
      doc.text(
        item.phoneNumbers[7].teacherName +
          "   " +
          item.phoneNumbers[7].phoneNumber,
        205,
        34,
        { align: "right" }
      );
      doc.text(
        item.phoneNumbers[8].teacherName +
          "   " +
          item.phoneNumbers[8].phoneNumber,
        205,
        40,
        { align: "right" }
      );
      ...
      doc.setFont("Helvetica", "", "bold");
      doc.setFontSize(25);
      doc.text("Friday", 14, 50 + (item.friday.length * 8 + 27) / 2, {
        angle: 90,
      });
      //Saturday Title
      doc.setFillColor(200, 200, 200);
      doc.rect(
        5,
        50 + item.friday.length * 8 + 2 + 10,
        13,
        item.saturday.length * 8 + 2,
        "F"
      );
      doc.setDrawColor(0, 0, 0);
      doc.setLineWidth(0.5);
      ...
      doc.text(
        ...
        14,
        50 +
          item.friday.length * 8 +
          2 +
          10 +
          (item.saturday.length * 8 + 38) / 2,
        { angle: 90 }
      );
      ...
      doc.rect(
        ...
        50 +
          ...
          2 +
          10 +
          item.saturday.length * 8 +
          2 +
          10,
        13,
        item.sunday.length * 8 + 2,
        "F"
      );
      doc.setDrawColor(0, 0, 0);
      doc.setLineWidth(0.5);
      ...
      for (let i = 0; i < item.saturday.length; i++) {
        doc.text(
          item.saturday[i].timeSlot,
          20,
          50 + item.friday.length * 8 + 2 + 10 + 6 + 8 * i
        );
        let required = "";
